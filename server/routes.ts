...
export function registerRoutes(app: Express): Server {
  ...
  app.post("/api/question-sets", async (req, res) => {
    ...
    try {
      ...
      questionSet.createdBy = req.user.id;
      
      const newQuestionSet = await storage.createQuestionSet(questionSet);
  ...
  app.get("/api/question-sets", async (req, res) => {
    ...
    try {
      const questionSets = await storage.getActiveQuestionSets();
  ...
  app.put("/api/question-sets/:id", async (req, res) => {
    ...
    try {
      ...
      const updatedQuestionSet = await storage.updateQuestionSet(id, data);
  ...
  app.delete("/api/question-sets/:id", async (req, res) => {
    ...
    try {
      const id = parseInt(req.params.id);
      await storage.deleteQuestionSet(id);
      res.sendStatus(200);
    } catch (error) {
  ...
  // Get questions for user from a specific question set
  app.get("/api/user/question-sets", async (req, res) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    
    try {
      const { setId, count } = req.query;
      
      if (!setId) {
        return res.status(400).json({ error: "Set ID is required" });
      }
      
      const questions = await storage.getQuestionsForUser(
        req.user.id, 
        parseInt(setId as string), 
        count ? parseInt(count as string) : 3
      );
